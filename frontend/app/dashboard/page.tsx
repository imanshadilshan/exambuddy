  ...
  const averagePercentage = attempts.length
    ? Math.round(
        attempts.reduce((sum, a) => {
          if (!a.total_questions || !a.marks_obtained) return sum
          return sum + (a.marks_obtained / a.total_questions) * 100
        }, 0) / attempts.length
      )
    : 0
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6 mb-8 shadow-sm">
          ...
          <p className="text-gray-600">Ready to ace your exams!</p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div className="bg-white rounded-lg p-6 border border-gray-200 shadow-sm">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <div className="text-xs text-gray-500 font-medium">Enrolled Courses</div>
                  <div className="text-2xl font-bold text-gray-900">{enrollments.courses.length}</div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg p-6 border border-gray-200 shadow-sm">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <div className="text-xs text-gray-500 font-medium">Enrolled Exams</div>
                  <div className="text-2xl font-bold text-gray-900">{enrollments.exams.length}</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <Link href="/student/my-courses" className="block bg-white rounded-lg p-6 border border-teal-200 shadow-sm hover:shadow-md transition-shadow cursor-pointer">
              ...
              <p className="text-sm text-gray-600">
                {attempts.length > 0
                  ? `${attempts.length} completed exam${attempts.length !== 1 ? 's' : ''}`
                  : 'View your exams and past results'}
              </p>
            </Link>

            <Link href="/student/rankings" className="block bg-white rounded-lg p-6 border border-gray-200 shadow-sm hover:shadow-md transition-shadow cursor-pointer">
            ...
            <Link href="/student/my-results" className="block bg-white rounded-lg p-6 border border-gray-200 shadow-sm hover:shadow-md transition-shadow cursor-pointer">
            ...
            <Link href="/profile" className="block bg-white rounded-lg p-6 border border-gray-200 shadow-sm hover:shadow-md transition-shadow cursor-pointer">
              <div className="w-12 h-12 bg-purple-100 rounded-lg flex items-center justify-center mb-3">
                <svg className="w-6 h-6 text-purple-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {/* Ranking Summary */}
        <div id="ranking-summary" className="bg-white border border-gray-200 rounded-xl p-8 shadow-sm mb-8">
          <h3 className="text-xl font-bold text-gray-900 mb-4">Ranking Summary</h3>
          {attempts.length === 0 ? (
        ...
        {/* Recent Activity */}
        <div id="recent-activity" className="bg-white border border-gray-200 rounded-xl p-8 shadow-sm">
          ...
          {loadingAttempts ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 text-sm mt-1">Start an exam to see your progress here</p>
            </div>
          ) : (
            <div className="space-y-3">
              {attempts.map((a) => (
